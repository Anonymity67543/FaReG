...
class data_model:
  ...
  def sample_X(self, n):
    ...
    group_alloc_1 = np.array([np.random.multinomial(1, self.group_perc_1) for i in range(X.shape[0])], dtype = float)
    group_label_1 = np.arange(len(self.group_perc_1))
    groups1 = np.array([np.dot(group_alloc_1[i], group_label_1) for i in range(X.shape[0])], dtype = int)
    X[:,-1] = groups1

    group_alloc_2 = np.array([np.random.multinomial(1, self.group_perc_2) for i in range(X.shape[0])], dtype = float)
    group_label_2 = np.arange(len(self.group_perc_2))
    groups2 = np.array([np.dot(group_alloc_2[i], group_label_2) for i in range(X.shape[0])], dtype = int)
    X[:,-2] = groups2

    # encode noise attributes (non-sensitive)
    X[:,-3] = list(islice(cycle([1,2,3,4,5]), n))
    X[:,-4] = pd.cut(X[:,-4], bins=[0, 0.25, 0.5, 0.75, float('Inf')], labels=[6,7,8,9])
    return X
  ...
  def compute_prob(self, X):
    ...
    for i in range(X.shape[0]):
      if (X[i, -1] == 0 and X[i, -2] == 0) or (X[i, -1] == 1 and X[i, -2] == 1):
